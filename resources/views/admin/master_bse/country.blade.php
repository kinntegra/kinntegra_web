...
<div class="panel-wrapper">
    ...
    <div class="container-fluid">
        ...
        <h3 class="section-title mb-4 d-flex justify-content-between">Country
                ...
                <div class="row flex-row-reverse ">
                            ...
                            <ul class="nav nav-tabs mb-4" id="lead-generation" role="tablist">
                                <li class="nav-item" role="presentation">
                                    <a class="nav-link active" data-toggle="tab" href="#admin-tax"
                                        role="tab" aria-selected="true">Country</a>
                                </li>
                            </ul>
                                ...
                                <div class="tab-pane fade show active" id="admin-tax" role="tabpanel">
                                    <div class="card small-card option-2 option-2 mb-4">
                                        ...
                                        <div class="card-body pt-4 pb-4">
                                            <form id="add-update-data" method="POST" action="{{ route('countries.store') }}">
                                            @csrf
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-xl-4 col-sm-12 fatca_code">
                                                        <div class="form-group mb-3 mb-sm-0">
                                                            <label>Fatca Code</label>
                                                            <input type="text" name="fatca_code" class="form-control" placeholder="Enter Fatca Code">
                                                        </div>
                                                    </div>
                                                </div>
                                                <button type="submit" class="btn btn-primary mt-4">Save</button>
                                                <a href="#" class="btn btn-primary mt-4" id="get_states" style="display: none;">Get States</a>
                                            </form>
                                        </div>
                                    </div>
                    ...
                    <div class="col-lg-3 col-md-5">
                        ...
                        <ul class="logs-list mt-4">
                            ...
                            <li>
                                <a href="javascript:void(0)" onclick="return viewEditFunction({{$country->id}});"><h4>{{$country->name}}</h4></a>
                            </li>
                            @endforeach
                        </ul>
                    </div>
...
<script>
    ...
    {
        $.get("/admin/master/countries?id="+$id, function(output, status){
            ...
            let name = data['name'];
            let code = data['code'];
            let fatca_code = data['fatca_code'];
            $("a#get_states").attr("href", "/admin/master/countries/"+id+"/states");
            $("#get_states").show();
            $("input[name=id]").val(id);
            $("input[name=name]").val(name);
            $("input[name=code]").val(code);
            $("input[name=fatca_code]").val(fatca_code);
    ...
    {
        let data = '';
        $.get("/admin/master/countries?name="+value, function(output, status){
            console.log(output);
